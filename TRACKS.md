...
Index tables and notes for Little Big Adventure 1 MIDI indices 0–32 as played
by [lba-midi-play](README.md).
...
`PERSO.C`, `PLAYFLA.C`, and `GAMEMENU.C` refer to twin-e, ScummVM, or the
original LBA1 game sources — not this repo.
...
indices. Engines such as twin-e call `music_play_midi(index, …)` with no string
labels in the HQR for “track name”.

Philippe Vachey released a commercial LBA1 OST with a small set of named album
tracks (on the order of nine pieces, not 33) —
[Bandcamp](https://philv.bandcamp.com/album/little-big-adventure-1-o-s-t).
Those titles are album cuts; mapping them one-to-one onto indices `0…32` is not
documented by Adeline and is partly interpretation.

There is no authoritative 33-entry title table from the 1994 assets. Informal
names in wikis or forums are community-derived. The table below uses scene /
role (from engine data and ScummVM’s scene IDs) as the most concrete in-game
labels without inventing a full soundtrack tracklist.
...
`PlayMusic()` in `AMBIANCE.C` routes tracks 1–9 to the CD drive (`PlayCdTrack()`)
when `CDEnable` is set, bypassing the MIDI files for those tracks. Tracks 10–32
always come from the MIDI HQR. The “CD audio track N” notes in the table match
the disc track numbers.
...
| 1 | 7 274 B | Scene header + code | Principal Island Ruins (17), White Leaf Desert Maze (57), Brundle Island teleportation (95, 99); intro dream sequence — before INTROD.FLA (hardcoded) | CD audio track 2 when CD enabled |
...
| 6 | 3 422 B | Scene header + code | Citadel Island Twinsen's house (5), Principal Island Library (10), Old Burg (13), inside Rabbibunny house (28), Stables (32), House with TV (58), Tippet Island Village (74); DEMO end credits (hardcoded) | CD audio track 7 when CD enabled |
...
| 9 | 21 980 B | Scene header + code | Principal Island Port Belooga (24), Tippet Island near Dino-Fly (78), Hamalayi Mountains Ski resort (96), Principal Island house in Port Belooga (102); options menu (`GAMEMENU.C` `OptionsMenu()`, non-CD) + credits loop (`PERSO.C`) | Byte-for-byte copy of track 8. CD audio track 10 when CD enabled |
...
| 26 | 322 B | Code only | FLA cutscene flute (`PLAYFLA.C`, `// fla flute`) | Identical between MIDI_MI and MIDI_SB |
...
| 31 | 166 B | Code only | Adeline logo screen (hardcoded in engine) | 14-note Vibraphone jingle. Identical between MIDI_MI and MIDI_SB. |
| 32 | 21 980 B | Scene header | Polar Island end scene (114), Citadel Island end sequence 1 (116) & end sequence 2 (117) | Finale / ending. Same bytes as tracks 8 and 9. |

> Scene 119 (Credits_List_Sequence) stores 255 (0xFF) at the music byte; twin-e
> reads it as −1 and treats that as no music / stop.
